...
const F_MIN = 1e-4;   // 100 kHz in GHz
const F_MAX = 67;
...
export function renderRuler (d) {
  const W = 356, H = 74;
  const x0 = 10, x1 = W - 10, span = x1 - x0;
  const axisY = 52;
  ...
  return `
<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Frequency coverage">
  ${bands.map(([lo, hi, name]) => {
    const bx = x0 + posOf(lo) * span, bw = (posOf(hi) - posOf(lo)) * span;
    return `<rect x="${bx.toFixed(1)}" y="6" width="${bw.toFixed(1)}" height="${axisY - 6}"
      fill="var(--text-faint)" fill-opacity=".07"/>
      <text x="${(bx + bw / 2).toFixed(1)}" y="14" font-size="7" fill="var(--text-faint)"
